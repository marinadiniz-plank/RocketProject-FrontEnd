...
export const useCrewman = (): [Data[], useCrewmanMethods] => {
	...
	const addCrewman = async (crewman: Data) => {
		await SubmitCrewman(crewman);
		getCrewmans();
	};
	const updateCrewman = async (id: number, crewman: Partial<Data>) => {
		await UpdateCrewman(id, crewman);
		getCrewmans();
	};
	const deleteCrewman = async (id: number) => {
		await DeleteCrewman(id);
		getCrewmans();
	};
