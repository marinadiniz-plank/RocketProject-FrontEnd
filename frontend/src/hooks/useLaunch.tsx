...
export const useLaunch = (): [Data[], useLaunchMethods] => {
	...
	const getLaunchs = async () =>
		setData(
			(await GetLaunch()).sort(
				(a: { id: number }, b: { id: number }) => a.id - b.id
			)
		);
	const addLaunch = async (launch: Data) => {
		await SubmitLaunch(launch);
		getLaunchs();
	};
	const updateLaunch = async (id: number, launch: Partial<Data>) => {
		await UpdateLaunch(id, launch);
		getLaunchs();
	};
